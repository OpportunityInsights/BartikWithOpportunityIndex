...
merge 1:1 czone year educ_coll_lt4yrs educ_coll_4yrs ageDecile using "$raw/input_BAR2"

drop _merge

merge m:1 czone year using "$derived/rentCosts"
drop if _merge == 2
...
merge m:1 cz using "$dropbox/outside/finer_geo/website/data/cz_covariates.dta"
drop if _merge == 2
drop _merge
...
save "$derived/aggregateBartikData_splitByAgeEduc", replace
