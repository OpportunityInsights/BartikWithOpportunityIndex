...
use C:\Users\per023\Documents\GitHub\gpss_replication\data\2000\input_BAR2, clear
...
program define test_and_compare, rclass
	btsls, z(z2) x($x) y($y) controls() ktype("tsls") weight_var($weight)
	local b_bar_lo_2sls_1 = r(beta)
	return scalar b_bar_lo_2sls_1 = `b_bar_lo_2sls_1'
	btsls, z(z2) x($x) y($y) controls($controls) ktype("tsls") weight_var($weight)
	...
	btsls, z("`1'") x($x) y($y) controls() ktype("mbtsls") weight_var($weight)
...
program define  liml_bootstrap, rclass
	gen ones = 1
	reghdfe  $y  ($x =  sh_ind_*) [aw=$weight], absorb( ones) keepsin estimator(liml) old
	local b_1 = _b[$x]
	reghdfe  $y $controls ($x =  sh_ind_*) [aw=$weight], absorb( ones) keepsin estimator(liml) old
	local b_2 = _b[$x]
	drop ones
...
program define test_and_compare_chao, rclass
	overid_chao, z(`1') x($x) y($y) controls(czone_*)  weight_var($weight)
	local b_1 = r(delta)
	return scalar b_1 = `b_1'
...
bootstrap b1_bartik = r(b_bar_2sls_1) b2_bartik = r(b_bar_2sls_2) diff_bartik = r(diff_bar_2sls) ///
  ...
  reps(`n'): test_and_compare  "`insts2'"
mat pval = r(table)
...
qui ivregress liml `y'  (`x' =  `insts2') [aw=`weight']
local b1_liml = string(_b[`x'], "%12.2f")
local se1_liml = "(" + string(_se[`x'], "%12.2f") + ")"
qui ivregress liml `y'  (`x' =  `insts2') `controls' [aw=`weight']
...
/*
capture bootstrap b1_hful = r(b_1) b2_hful = r(b_2) diff_hful = r(diff), reps(`n'): test_and_compare_chao "`insts2'"
mat b = e(b)
mat V = vecdiag(e(V))
...
capture erase "$data_path/results/bar_results.tex"
file open fh using "$data_path/results/bar_results.tex", write replace
