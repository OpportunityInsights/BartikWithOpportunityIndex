...
global main = "/nfs/home/P/per023/shared_space/per023_proj"
...
merge 1:1 year serial pernum using "${main}/raw/IPUMS_geo", nogen keep(3) keepusing(countyicp countyfip cntygp98 puma)
...
egen panelVar = group(`geo' ageDecile educ_coll_lt4yrs educ_coll_4yrs)
sort panelVar year
tsset panelVar year, delta(`delta')
order panelVar year, first
...
collapse (sum)  indwt = perwt_cz (firstnm) `geo' ind`ind_digits' if age>=18 & full_time==1, by(`geo'_ind`ind_digits' year ageDecile educ_coll_lt4yrs educ_coll_4yrs) //Collapse to summed weights by Industry-Location-Year, using only 18+ full-time individuals
